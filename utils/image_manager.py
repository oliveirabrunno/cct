...
class ImageManager:
    ...
    def __init__(self):
        ...
        # Rastreia paths já usados nesta sessão de carrossel para evitar repetição
    ...
    async def get_player_image(
        ...
        # 1. Cache local — foto ainda não usada nesta sessão
        ...
        # 2. Flickr — fotos CC recentes de torneios (precisa FLICKR_API_KEY)
        if _FLICKR_AVAILABLE:
            result = await self._try_flickr(player_name, image_type)
            if result:
                return result

        # 3. Wikipedia profile — headshot confiável e identificado do jogador
        #    Sempre o jogador certo; usa para foto_clean/headshot
        if image_type in ("headshot", "any"):
            result = await self._try_wikipedia_profile(player_name, image_type)
            if result:
                return result

        # 4. Wikimedia com intitle: — busca por nome no arquivo + temporada
        result = await self._try_wikimedia(player_name, image_type)
        if result:
            return result

        # 5. Fallback: Wikipedia profile mesmo pedindo action (melhor que nada)
        if image_type == "action":
            result = await self._try_wikipedia_profile(player_name, "action")
            if result:
                return result
        ...
        log.warning(f"Nenhuma imagem nova para '{player_name}' — placeholder")
        return self._get_placeholder(player_name)

    # ── Fontes individuais ────────────────────────────────────────────────────

    async def _try_flickr(self, player_name: str, image_type: str) -> dict | None:
        try:
            from utils.image_sources.flickr import search_player_images as flickr_search
            for photo in flickr_search(player_name, count=4):
                try:
